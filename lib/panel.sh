...
# - Egg diimport langsung menggunakan Laravel Model
# - Tidak membutuhkan API token
# - Egg yang sudah ada tidak dibuat duplikat
# ============================================================
...
import_pterodactyl_eggs() {
    ...
    if ! php -r '
        ...
        foreach ($files as $file) {
            if (!is_file($file)) {
                fwrite(STDERR, "FILE_NOT_FOUND:$file\n");
                exit(1);
            }
            ...
            if (!isset($data["meta"]["version"])) {
                fwrite(STDERR, "INVALID_EGG_VERSION:$file\n");
                exit(1);
            }

            if ($data["meta"]["version"] !== "PTDL_v2") {
                fwrite(
                    STDERR,
                    "UNSUPPORTED_EGG_VERSION:" .
                    $data["meta"]["version"] .
                    "\n"
                );
            ...
            if (empty($data["name"])) {
                fwrite(STDERR, "EGG_NAME_MISSING:$file\n");
                exit(1);
            }

            if (empty($data["docker_images"])) {
                fwrite(STDERR, "DOCKER_IMAGES_MISSING:$file\n");
                exit(1);
            }

            if (!isset($data["scripts"]["installation"])) {
                fwrite(STDERR, "INSTALL_SCRIPT_MISSING:$file\n");
                exit(1);
            }
        }

        exit(0);
    ' "$egg_dir/egg-bot-vyro.json" "$egg_dir/egg-python.json"; then
...
require __DIR__ . '/../../var/www/pterodactyl/vendor/autoload.php';
...
function normalize_json_string(mixed $value): ?string
{
    if ($value === null) {
        return null;
    }
...
/**
 * ==========================================================
 * Dynamic model fill helper
 * ==========================================================
 *
 * Berfungsi supaya kompatibel dengan perubahan field model
 * pada versi Pterodactyl yang berbeda.
 *
 */
...
function get_or_create_nest(
    ...
    if ($nest) {
        output_line("[OK] Nest sudah ada: {$name}");
        return $nest;
    }
    ...
    output_line("[OK] Nest dibuat: {$name}");
...
function import_variables(
    ...
    if (empty($variables)) {
        output_line("[INFO] Egg tidak memiliki variable.");
        return;
    }
    ...
    foreach ($variables as $variable) {
        ...
        $fieldMap = [
            'name' => $variable['name'] ?? $envVariable,
...
function import_egg(
    ...
    $scriptContainer = $installation['container'] ?? null;
    $scriptEntry = $installation['entrypoint'] ?? null;

    $configFiles = normalize_config_value(
        $config['files'] ?? '{}',
        '{}'
    );

    $configStartup = normalize_config_value(
        $config['startup'] ?? '{}',
        '{}'
    );

    $configLogs = normalize_config_value(
        $config['logs'] ?? '{}',
        '{}'
    );
    ...
    if ($configStop !== null) {
        $configStop = (string) $configStop;
    }

    $fileDenylist = $data['file_denylist'] ?? [];
    ...
    $scriptPrivileged =
        $data['scripts']['installation']['privileged']
        ?? false;
    ...
    /**
     * Author biasanya bukan fillable di Egg model,
     * jadi kita set langsung.
     */
    ...
    /**
     * update_url tidak selalu mass assignable.
     */

    if ($metaUpdateUrl !== null) {
        $egg->update_url = $metaUpdateUrl;
        $egg->save();
    }
...
$nodejsFile = $argv[1] ?? null;
$pythonFile = $argv[2] ?? null;

if (!$nodejsFile || !$pythonFile) {
    fail_import(
...
PHP
    ...
    # ========================================================
    #
    # Karena importer berada di /tmp, Laravel bootstrap path
    # di atas tidak boleh bergantung pada relative ../../.
    #
    # Kita buat ulang dengan path absolut.
    #

    sed -i \
        "s#require __DIR__ . '/../../var/www/pterodactyl/vendor/autoload.php';#require '/var/www/pterodactyl/vendor/autoload.php';#" \
        "$importer"

    sed -i \
        "s#\$app = require __DIR__ . '/../../var/www/pterodactyl/bootstrap/app.php';#\$app = require '/var/www/pterodactyl/bootstrap/app.php';#" \
        "$importer"
...
SQL
    ...
    # - artisan
    #
    # Jadi tabel nests, eggs dan egg_variables sudah tersedia.
    #
...
SERVICE
    ...
    info "Memeriksa Egg yang terpasang..."
    ...
    egg_count="$(
        ...
        php artisan tinker --execute="
            echo \Pterodactyl\Models\Egg::whereIn(
                'name',
                ['XenOfficial', 'Python']
            )->count();
        " 2>/dev/null |
    ...
    if [[ -n "$egg_count" && "$egg_count" -ge 2 ]]; then

        log "Egg XenOfficial + Python: OK"

    else

        warn "Tidak semua Egg auto-import terdeteksi."
        failed=1
    ...
    if [[ "$failed" -eq 0 ]]; then
        ...
        info "Egg NodeJS  : XenOfficial"
        info "Egg Python  : Python"
