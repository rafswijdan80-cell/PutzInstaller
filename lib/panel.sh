...
# - Tidak membuat Nest duplikat
# - Variable tidak duplikat
# - Author Egg/Nest wajib valid
# - Nama Egg dibaca langsung dari JSON
# - Compatible dengan PTDL_v2
# - Permission Pterodactyl diperbaiki
# - Nginx menggunakan /public
# - HTTP 403 protection
# ============================================================
...
import_pterodactyl_eggs() {
    ...
    if [[ ! -f /var/www/pterodactyl/vendor/autoload.php ]]; then
        error "Vendor Composer Pterodactyl tidak ditemukan."
        return 1
    fi
...
function set_attribute_if_exists(
    Model $model,
    string $field,
    mixed $value
): void {
...
function get_or_create_nest(
    ...
    if ($nest) {
        ...
        /*
         * Fix description jika kosong.
         */

        if (
            empty(
                $nest->getAttribute('description')
            )
        ) {
...
function import_variables(
    ...
    foreach ($variables as $variable) {
        ...
        if ($existing) {
            ...
            output_line(
                "[SKIP] Variable sudah ada: {$envVariable}"
            );
        ...
        output_line(
            "[OK] Variable dibuat: {$envVariable}"
        );
    }
    ...
    if ($skipped > 0) {
        ...
        output_line(
            "[INFO] Variables skipped: {$skipped}"
        );
    }
}
...
function import_egg(
    ...
    if (
        ...
        fail_import(
            "Author Egg tidak valid untuk {$name}: {$author}"
        );
    }
     ...
     *
     * Egg dengan nama sama tidak dibuat ulang,
     * walaupun berada di Nest berbeda.
     * ======================================================
     */
    ...
    if ($existing) {
        ...
        /*
         * Pastikan variables tetap lengkap.
         */
        ...
        output_line(
            "[SKIP] Egg sudah ada secara global: {$name}"
        );


        output_line(
            "[INFO] Egg ID: {$existing->id}"
        );


        output_line(
            "[INFO] Nest ID: {$existing->nest_id}"
        );
...
PHP
    ...
    if [[ "$failed" -eq 0 ]]; then
        ...
        log "Auto-import Egg selesai tanpa error."
    ...
    warn "Auto-import Egg selesai tetapi verifikasi menemukan masalah."
